...
namespace ToracLibrary.Core.DataProviders.EntityFrameworkDP
{
    ...
    public class EntityFrameworkDP<TEfContextType> : IDisposable where TEfContextType : DbContext, new()
    {
        ...
        public bool ColumnIsAutoSeedLookup(string TableName, string ColumnName)
        {
            ...
            if (!ColumnIsAutoSeed.HasValue)
            {
                ...
                throw new NullReferenceException(string.Format("Can't Find Column '{0}' In The Table '{1}'. Please Correct The Parameters Of Method", ColumnName, TableName));
            }
